...
var PACKET_TYPE_ENTER = 'enter';
var PACKET_TYPE_BEGIN = 'begin';
var PACKET_TYPE_KILL = 'kill';
var PACKET_TYPE_CHECK = 'check';
var PACKET_TYPE_VOTE = 'vote';
var PACKET_TYPE_NIGHT = 'night';
var PACKET_TYPE_DAY = 'day';
var PACKET_TYPE_END = 'end';
...
function Tunnel(serviceUrl) {
    ...
    function sendPacket(packet) {
        ...
        if (encodedPacket[0] == "ping") {
            wx.sendSocketMessage({
                data: encodedPacket[0],
                fail: handleSocketError,
            });
        }
        else {
            wx.sendSocketMessage({
                data: encodedPacket[1],
                fail: handleSocketError,
            });
        }
    }
    ...
    function resolvePacket(raw) {
        var messageData = JSON.parse(raw);
        var packet = { type: messageData.type };

        if (messageData.content) {
            try {
                packet.content = messageData.content;
            } catch (e) {}
        }
        ...
        case PACKET_TYPE_ENTER:
            handleEnterPacket(packet);
            break;
        case PACKET_TYPE_BEGIN:
           handleBeginPacket(packet);
            break; 
        ...
        case PACKET_TYPE_CHECK:
           handleBeginPacket(packet);
            break; 
        case PACKET_TYPE_VOTE:
           handleBeginPacket(packet);
            break; 
        case PACKET_TYPE_NIGHT:
           handleBeginPacket(packet);
            break; 
        case PACKET_TYPE_DAY:
           handleBeginPacket(packet);
            break; 
    ...
    function handleEnterPacket(packet) {
        dispatchEscapedEvent(packet.type, packet.content);
    }

    function handleBeginPacket(packet) {
        dispatchEscapedEvent(packet.type, packet.content);
    }
